...
public class MyModel extends Observable implements IModel {
            ...
            Client client = new Client(InetAddress.getLocalHost(), 5400, new IClientStrategy() {
                ...
                public void clientStrategy(InputStream inFromServer, OutputStream outToServer) {
                    try {
                        ...
                        byte[] decompressedMaze = new byte[8+ mazeSize[0] * mazeSize[1]];
            ...
            Client client = new Client(InetAddress.getLocalHost(), 5401, new IClientStrategy() {
                ...
                public void clientStrategy(InputStream inFromServer, OutputStream outToServer) {
                    try {
                        ...
                        toServer.flush();
                        Maze maze = firstMaze;
                        maze.setP_start(charRow,charCol);
                        ...
                        if(x == "solve") {
                            for (int i = 0; i < mazeSolutionSteps.size(); i++) {
                                solutionArray[0][i] = ((MazeState)(mazeSolutionSteps.get(i))).getCurrPosition().getRowIndex();
                                solutionArray[1][i] = ((MazeState)(mazeSolutionSteps.get(i))).getCurrPosition().getColumnIndex();
                            }
                        }
                        else if (x == "clue"){
                            int i = 1;
    ...
    public void setFirstMaze(Maze m){
        ...
        for (int i = 0; i < row; i++)
            for (int j = 0; j < col; j++)
                firstMaze.setValue(i,j,m.getValue(i, j));
    }
    /**
     * setter
     */
